...
template <typename T1, typename T2> class vtkGenerator {
  ofstream out;
...
public:
  ...
  void printInVTK(const FixedSizeMeshContainer<T1> &coords,
                  const VariableSizeMeshContainer<T2> &topoEN) {
    ...
    out << "CELLS " << topoEN.getBlockNumber() << " "
        << topoEN.getTotalSize() + topoEN.getBlockNumber() << endl;
    ...
    for (size_t i = 0; i < topoEN.getBlockNumber(); ++i) {
      ...
      case 3:
        out << CELL_TYPE_TRIANGLE << " ";
        break;
      case 4:
        out << CELL_TYPE_RECTANGLE << " ";
        break;
      }
    }
