...
// default constructor
template <typename T> FixedSizeMeshContainer<T>::FixedSizeMeshContainer() {
  blockSize = 0;
}

template <typename T>
FixedSizeMeshContainer<T>::FixedSizeMeshContainer(size_t num)
    : blockSize(num){};

template <typename T>
FixedSizeMeshContainer<T>::FixedSizeMeshContainer(const vector<T> &source,
                                                  size_t blockSize)
    : blockSize(blockSize) {
...
FixedSizeMeshContainer<T>::FixedSizeMeshContainer(
    const FixedSizeMeshContainer<T> &source)
...
template <typename T> void FixedSizeMeshContainer<T>::reserve(size_t size) {
  V.reserve(size);
}

template <typename T> void FixedSizeMeshContainer<T>::shrink_to_fit() {
  V.shrink_to_fit();
}

template <typename T>
FixedSizeMeshContainer<T> &
FixedSizeMeshContainer<T>::operator=(const FixedSizeMeshContainer<T> &extra) {
...
template <typename T> size_t FixedSizeMeshContainer<T>::getBlockNumber() const {
  return V.size() / blockSize;
}

template <typename T> size_t FixedSizeMeshContainer<T>::getBlockSize() const {
  return blockSize;
}

template <typename T> size_t FixedSizeMeshContainer<T>::getTotalSize() const {
  return V.size();
}
