...
// matrix interface.
template <typename T> class VariableSizeMeshContainer {
    size_t blockNumber;
...
template <typename T> int VariableSizeMeshContainer<T>::count_digit(int x) const {
    int i = 0;
    if (x == 0)
        return 1;
    while (x > 0) {
...
// default constructor
template <typename T> VariableSizeMeshContainer<T>::VariableSizeMeshContainer() { blockNumber = 0; }

// Creates VariableSizeMeshContainer from source vector.
template <typename T>
VariableSizeMeshContainer<T>::VariableSizeMeshContainer(
    const vector<T> &source,
    const vector<int> &blockSizes) // number is size of each vector's block
{
...
template <typename T>
VariableSizeMeshContainer<T>::VariableSizeMeshContainer(
    const VariableSizeMeshContainer<T> &source) // number is size of each vector's block
{
...
// Can only add elements of vectors which sizes are divided without remainder
// into {M}
template <typename T> bool VariableSizeMeshContainer<T>::add(const vector<T> &extra, const vector<int> &blockSizes) {
...
// Add another VariableSizeMeshContainer elements to our container
template <typename T> bool VariableSizeMeshContainer<T>::add(const VariableSizeMeshContainer<T> &extra) {
...
template <typename T> T *VariableSizeMeshContainer<T>::operator[](size_t i) {
    if (i >= blockNumber) {
...
template <typename T> const T *VariableSizeMeshContainer<T>::operator[](size_t i) const {
    if (i >= blockNumber) {
        cerr << "Error: Index out of bounds" << endl;
        return nullptr;
    } else {
...
template <typename T> size_t VariableSizeMeshContainer<T>::getBlockNumber() const { return blockNumber; }

template <typename T> size_t VariableSizeMeshContainer<T>::getBlockSize(size_t i) const { return IA[i + 1] - IA[i]; }

template <typename T> size_t VariableSizeMeshContainer<T>::getTotalSize() const { return V.size(); }

template <typename T> void VariableSizeMeshContainer<T>::clear() {
    IA.clear();
    V.clear();
}
