...
void axpby(std::vector<VT> &x, const std::vector<VT> &y, const ST &a,
           const ST &b, size_t threadsNumber = 4) {
  size_t size = x.size();
  if (size != y.size()) {
...
VT dot(const std::vector<VT> &x, const std::vector<VT> &y,
...
template <typename VT>
void multiply(const std::vector<VT> &x, std::vector<VT> &result, VT scalar,
              size_t threadsNumber = 4) {
  double start = omp_get_wtime();
#pragma omp parallel for num_threads(threadsNumber)
...
std::vector<double> conGradSolver(Sparse<M> &A, const std::vector<V> &b,
                                  size_t threadsNum = 4, size_t n_max = 100,
  ...
  if (A.getDenseColumns() != b.size()) {
    ...
    std::vector<double> r_prev =
        b; // we think that x0 = (0,0,0,0,0,0 ... 0)^(T) -> b - Ax = b
    ...
    do {
      ...
      std::cout << "|" << setw(11) << k << "|" << setw(12) << fixed
                << setprecision(7) << norm(r_prev) / b_norm << "|" << endl;
    ...
    std::cout << std::endl;
    std::cout << "Conjugate gradient method solution total time: "
              << conGradSolverTime << std::endl;
    std::cout << std::endl;
  }
