...
public:
  virtual void spmv(const std::vector<T> &, std::vector<T> &result,
                    double &timeSPvm, size_t threadsNumber = 4) const = 0;
  virtual void spmv(const T *, std::vector<T> &result, size_t, double &timeSPvm,
                    size_t threadsNumber = 4) const = 0;
  ...
  size_t getDenseRows() const { return denseRows; }

  size_t getDenseColumns() const { return denseColumns; }
...
template <typename T> class SparseELL : public Sparse<T> {
...
public:
  SparseELL(const std::vector<T>
                &diagonal) // vector is put on primary diagonal,no padding
  {
  ...
  SparseELL(T **dense, size_t rows,
            size_t columns) // from dense(needed in solver)
  {
    this->valuesSize = 0;
    this->denseRows = rows;
  ...
  void spmv(const std::vector<T> &x, std::vector<T> &result, double &timeSPmv,
            size_t threadsNumber = 4) const override {
    if (x.size() != this->denseColumns) {
      std::cerr << "Incompatible sizes in spmv!" << std::endl;
    } else {
      double start = omp_get_wtime();
#pragma omp parallel for num_threads(                                          \
    threadsNumber) // TODO: ask about template reduction
      for (OPENMP_INDEX_TYPE i = 0; i < this->denseRows; ++i) {
        result[i] = 0;

        for (OPENMP_INDEX_TYPE j = i * this->rowOffset;
             j < (i + 1) * this->rowOffset; ++j) {
  ...
  void spmv(const T *x, std::vector<T> &result, size_t xSize, double &timeSPmv,
            size_t threadsNumber = 4) const override {
...
#pragma omp parallel for num_threads(threadsNumber)
      for (OPENMP_INDEX_TYPE i = 0; i < this->denseRows; ++i) {
        ...
        for (OPENMP_INDEX_TYPE j = i * rowOffset; j < (i + 1) * rowOffset;
             ++j) {
          result[i] += x[this->JA[j]] * this->A[j];
        }
      }
  ...
  void setValues(const std::vector<T> &a) override {
    if (a.size() != this->getValuesSize()) {
      ...
      for (size_t i = 0, j = 0; j < a.size();
           i++) // i - for A, j - for vector "a"
      {
        if (this->A[i] != 0) {
          this->A[i] = a[j];
          j++;
  ...
  size_t getValuesSize() const override { return this->valuesSize; }
  ...
  std::vector<T> getDiagonal() const override {
    ...
    for (size_t i = 1; i < this->denseRows + 1; ++i) {
      for (size_t j = (i - 1) * rowOffset; j < i * rowOffset; ++j) {
        if ((this->JA[j] == i - 1) &&
            (this->A[j] !=
             0)) // avoiding padding : when padding index is index of diagonal
        {
...
template <typename T> class SparseCSR : public Sparse<T> {
...
#pragma omp parallel for num_threads(threadsNumber)
  ...
  size_t getValuesSize() const override { return this->A.size(); }
...
template <typename T> class SparseCOO : public Sparse<T> {
