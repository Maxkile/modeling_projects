...
public:
    virtual void spmv(const std::vector<T> &, std::vector<T> &result, double &timeSPvm, NodesInfo *nodesInfo,
                      size_t threadsNumber = 4) const = 0;
    virtual void spmv(const T *, std::vector<T> &result, size_t, double &timeSPvm, NodesInfo *nodesInfo,
                      size_t threadsNumber = 4) const = 0;
...
public:
    ...
    void spmv(const std::vector<T> &x, std::vector<T> &result, double &timeSPmv, NodesInfo *nodesInfo,
...
#pragma omp parallel for num_threads(threadsNumber) // TODO: ask about template reduction
    ...
    void spmv(const T *x, std::vector<T> &result, size_t xSize, double &timeSPmv, NodesInfo *nodesInfo,
              size_t threadsNumber = 4) const override {
    ...
    std::vector<T> getDiagonal() const override {
        ...
        for (size_t i = 1; i < this->denseRows + 1; ++i) {
            for (size_t j = (i - 1) * rowOffset; j < i * rowOffset; ++j) {
                if ((this->JA[j] == i - 1) &&
                    (this->A[j] != 0)) // avoiding padding : when padding index is index of diagonal
                {
