 ...
 * id -> submesh coords
 */
pair<size_t, vector<int>> decomp::decomposeMesh(int Nx, int Ny, int Px, int Py,
                                                int px, int py) {
  ...
  if ((px >= Px) || (py >= Py) || (px < 0) || (py < 0)) {
    ...
    if ((px < xLeft) &&
        (py < yLeft)) // add left element to coord[x] and coord[y]
    {
 ...
 */
vector<pair<size_t, vector<int>>> decomp::decomposeMesh(int Nx, int Ny, int Px,
                                                        int Py) {
 ...
 */
size_t
decomp::getSubmeshIdByCoords(int x, int y,
 ...
 */
bool decomp::isHalo(int x, int y,
                    const vector<pair<size_t, vector<int>>> &submeshes, int Nx,
                    int Ny, size_t current_id) {
  if (x >= 0 && y >= 0 && x < Nx && y < Ny) {
    return !(decomp::getSubmeshIdByCoords(x, y, submeshes, Nx, Ny) ==
             current_id);
  } else {
    return false;
  }
 ...
 */
bool decomp::isInterface(int x, int y,
                         const vector<pair<size_t, vector<int>>> &submeshes,
                         int Nx, int Ny, size_t current_id) {
  if (!isHalo(x, y, submeshes, Nx, Ny, current_id) ||
      !(x >= 0 && y >= 0 && x < Nx && y < Ny)) // not halo
  {
    if (isHalo(x + 1, y, submeshes, Nx, Ny, current_id) ||
        isHalo(x, y + 1, submeshes, Nx, Ny, current_id) ||
        isHalo(x + 1, y + 1, submeshes, Nx, Ny, current_id) ||
        isHalo(x - 1, y, submeshes, Nx, Ny, current_id) ||
        isHalo(x, y - 1, submeshes, Nx, Ny, current_id) ||
        isHalo(x - 1, y - 1, submeshes, Nx, Ny, current_id) ||
        isHalo(x - 1, y + 1, submeshes, Nx, Ny, current_id) ||
        isHalo(x + 1, y - 1, submeshes, Nx, Ny, current_id)) {
 ...
 */
void decomp::addHaloNodes(int x, int y,
                          const vector<pair<size_t, vector<int>>> &submeshes,
                          int Nx, int Ny, size_t current_id, set<int> &haloes) {
  for (int i = -1; i <= 1; ++i) {
    for (int j = -1; j <= 1; ++j) {
      int newX = x + i;
      int newY = y + j;
      if (newX >= 0 && newY >= 0 && newX < Nx && newY < Ny &&
          isHalo(newX, newY, submeshes, Nx, Ny, current_id)) {
 ...
 */
void decomp::formPart(vector<int> &part, const vector<int> &nodes,
                      const vector<pair<size_t, vector<int>>> &submeshes,
                      int Nx, int Ny) {
  part.clear();
...
void decomp::getGlobalIndexes(map<int, int> &G2L, vector<int> &global) {
  ...
  for (map<int, int>::iterator map_iter = G2L.begin(); map_iter != G2L.end();
       ++map_iter) // forming global
  {
